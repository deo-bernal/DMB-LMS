...
namespace Dmb.Lms.Service.Interface;
...
{
    Task<FileUploadResult> UploadAsync(string prefix, string fileName, string contentType, Stream content, CancellationToken cancellationToken = default);
    Task<string?> SignAsync(string storagePath, CancellationToken cancellationToken = default);
}
